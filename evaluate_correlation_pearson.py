...
def process_file(file_path, steam_gt, stock_gt):
    """
    개별 결과 파일을 처리하여 상관계수(Pearson)를 계산합니다.
    """
    try:
        ...
        # Pearson 상관계수 계산
        # Steam
        corr_steam, p_steam = pearsonr(merged_steam['Purchase_Ratio'], merged_steam['Positive_Ratio'])
        
        # Stock
        # merge_asof 결과에서 NaN이 있을 수 있으므로 제거
        merged_stock = merged_stock.dropna(subset=['Stock_Price'])
        if len(merged_stock) < 2:
            corr_stock, p_stock = np.nan, np.nan
        else:
            corr_stock, p_stock = pearsonr(merged_stock['Purchase_Ratio'], merged_stock['Stock_Price'])
        ...
        return {
            ...
            'Num_Data_Points': len(merged_steam), # Steam 기준 매칭 수
            'Note': 'Success'
        }
...
def main():
    parser = argparse.ArgumentParser(description="Pearson Correlation Evaluation Script")
    ...
    parser.add_argument("--output_csv", type=str, default="results/result_pearson.csv", help="결과 요약 저장 경로")
    parser.add_argument("--period", type=str, choices=['daily', 'weekly'], default="daily", help="주가 데이터 기간 설정 (daily: 당일 종가, weekly: 7일 평균)")
    
    args = parser.parse_args()
    
    print(f"--- Pearson Correlation Evaluation ---")
    ...
    # result_pearson.csv가 이미 있다면 제외 (자기 자신을 읽는 것 방지)
    # 또한 이전의 result.csv 등도 제외하는 게 좋을 수 있지만, 단순히 결과 파일만 피하면 됩니다.
    csv_files = [f for f in csv_files if os.path.abspath(f) != os.path.abspath(args.output_csv)]
    # 또한 result.csv (Spearman 결과)도 입력으로 쓰면 안되므로 제외하는 로직 추가
    csv_files = [f for f in csv_files if 'result.csv' not in f and 'result_pearson.csv' not in f]
